...
<div>
	...
	<div class="list-preview" :style='{"margin":"10px auto","overflow":"hidden","flexWrap":"wrap","background":"none","display":"block","width":"1200px","position":"relative"}'>
		...
		<div class="category-2" :style='{"width":"110px","margin":"10px 0 0","float":"left","flexDirection":"column","display":"flex","height":"auto"}'>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"2px solid #a4d9b7","padding":"10px 0","margin":"10px 0 0 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 130px)","float":"right","height":"auto"}'>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>帖子标题</div>
        <el-input v-model="formSearch.tiezibiaoti" placeholder="帖子标题" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>帖子分类</div>
        <el-select v-model="formSearch.tiezifenlei" placeholder="请选择帖子分类" :clearable="true">
          <el-option v-for="(item, index) in tiezifenleiOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#29292a","display":"inline-block"}'>标签</div>
	  ...
	  <el-button v-if="isAuth('tiezixinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"10px","outline":"none","color":"#fff","borderRadius":"10px","background":"#55b44b","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/tiezixinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"calc(100% - 130px)","float":"right","margin":"20px 0"}'>
		...
		<div class="list2 index-pv1" :style='{"border":"2px solid #a4d9b7","padding":"20px 0px 0 0px","overflow":"hidden","borderRadius":"8px","background":"#fff","width":"100%","height":"auto"}'>
			<div :style='{"cursor":"pointer","margin":"0 0 20px 2%","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"48%","fontSize":"0","float":"left","height":"150px"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"boxShadow":"8px 0px 0 rgba(146, 181, 164,.5)","borderColor":"rgba(146, 181, 164,.8)","objectFit":"contain","borderRadius":"10px 40px 40px 10px","borderWidth":"1px 12px 1px 1px","width":"240px","borderStyle":"solid","float":"left","height":"100%"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
				<img :style='{"boxShadow":"8px 0px 0 rgba(146, 181, 164,.5)","borderColor":"rgba(146, 181, 164,.8)","objectFit":"contain","borderRadius":"10px 40px 40px 10px","borderWidth":"1px 12px 1px 1px","width":"240px","borderStyle":"solid","float":"left","height":"100%"}' v-else :src="baseUrl + (item.tupian?item.tupian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"padding":"12px","margin":"0 20px 0 40px","overflow":"hidden","borderRadius":"8px","background":"#eff8f2","width":"calc(100% - 300px)","position":"relative","float":"right","height":"auto"}'>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.tiezibiaoti}}</div>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.tiezifenlei}}</div>
					<div :style='{"width":"100%","margin":"0px","lineHeight":"32px","fontSize":"16px","color":"#333","background":"none"}' class="name ">{{item.nicheng}}</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/tiezifenlei/tiezifenlei').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.indexQueryCondition) {
          searchWhere.tiezibiaoti = '%' + this.indexQueryCondition + '%';
          this.indexQueryCondition = '';
        }
        if (this.formSearch.tiezibiaoti != '') searchWhere.tiezibiaoti = '%' + this.formSearch.tiezibiaoti + '%';
        if (this.formSearch.tiezifenlei != '') searchWhere.tiezifenlei = this.formSearch.tiezifenlei;
        if (this.formSearch.biaoqian != '') searchWhere.biaoqian = '%' + this.formSearch.biaoqian + '%';
        params['sort'] = 'fabushijian';
        params['order'] = 'desc';
	      params['sfsh'] = '是';
        if (this.curFenlei != '全部') searchWhere.tiezifenlei = this.curFenlei;
        this.$http.get('tiezixinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/tiezixinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
