...
<div>
	...
	<div class="detail-preview" :style='{"width":"1200px","padding":"0 0 20px","margin":"0px auto","position":"relative","background":"none"}'>
		<div class="attr" :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px 0 0 0","overflow":"hidden","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","position":"relative","justifyContent":"space-between","height":"auto"}'>
			<el-carousel :style='{"width":"500px","margin":"0px 0","borderRadius":"0px","height":"500px","order":"1"}' trigger="click" indicator-position="inside" arrow="always" type="default" direction="horizontal" height="500px" :autoplay="true" :interval="3000" :loop="true">
				<el-carousel-item :style='{"borderRadius":"0px","width":"100%","height":"100%"}' v-for="item in detailBanner" :key="item.id">
					<el-image :style='{"border":"1px solid #daf4e3","width":"100%","objectFit":"contain","borderRadius":"0px","background":"#fff","height":"100%"}' v-if="item.substr(0,4)=='http'" :src="item" fit="cover" class="image"></el-image>
					<el-image :style='{"border":"1px solid #daf4e3","width":"100%","objectFit":"contain","borderRadius":"0px","background":"#fff","height":"100%"}' v-else :src="baseUrl + item" fit="cover" class="image"></el-image>
				</el-carousel-item>
			</el-carousel>
			...
			<div class="info" :style='{"minHeight":"570px","padding":"0 20px 20px","margin":"0px","borderRadius":"8px","background":"#f9f9f9","width":"600px","height":"auto","order":"2"}'>
				...
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"left","width":"130px","fontSize":"14px","lineHeight":"24px","fontWeight":"bold","height":"40px"}'>帖子分类</div>
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.tiezifenlei}}</div>
				</div>
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"left","width":"130px","fontSize":"14px","lineHeight":"24px","fontWeight":"bold","height":"40px"}'>标签</div>
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.biaoqian}}</div>
				</div>
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"left","width":"130px","fontSize":"14px","lineHeight":"24px","fontWeight":"bold","height":"40px"}'>发布时间</div>
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.fabushijian}}</div>
				</div>
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					...
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.yonghuming}}</div>
				</div>
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"left","width":"130px","fontSize":"14px","lineHeight":"24px","fontWeight":"bold","height":"40px"}'>昵称</div>
					<div  :style='{"width":"498px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#666"}'>{{detail.nicheng}}</div>
				</div>
				<div class="item" :style='{"padding":"4px","margin":"0 0 4px 0","borderColor":"rgba(85, 180, 75,.4)","background":"none","borderWidth":"0 0 1px 0 ","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
				...
				<div class="btn" :style='{"padding":"10px 0 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0px 5px 5px 0","outline":"none","color":"#383838","borderRadius":"4px","background":" #a8c2e5","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('tiezixinxi','审核')" @click="onSH" type="warning">审核</el-button>
				</div>
			</div>
		...
		<el-tabs class="detail" :style='{"border":"2px solid #a4d9b7","boxShadow":"none","padding":"20px","margin":"20px 0 0 0","borderRadius":"8px","background":"#fff"}' v-model="activeName" type="border-card">
			<el-tab-pane label="帖子内容" name="first">
				<div v-html="detail.tiezineirong"></div>
			</el-tab-pane>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.detail.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.tiezibiaoti;
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'tiezixinxi', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      thumbsupOrCrazily(type) {
        ...
        this.$http.post('tiezixinxi/update', this.detail).then(res => {});
      },
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'tiezixinxi', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'tiezixinxi', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discusstiezixinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        let sensitiveWords = "装逼,蛋疼,你妹,绿茶婊,屌爆了,你妈的,去死吧,吃屎吧,妈妈的,奶奶的,操,杀死你,台独,黄,毒,赌,黑鬼,重男轻女,滚粗,算命,抢,装13,大男人,小女人,洋鬼子,回回,高丽棒子,杂种,男尊女卑,东亚病夫,xx天见效,微信,QQ,寂寞,联系,操,傻逼,煞笔,智障,脑残,神经病,王八蛋,傻狗,约";
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusstiezixinxi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
